...
import { IPurchaseProduct } from "../interfaces/index";
import {
...
export const purchasesController = {
  create: (async (req: Request, res: Response) => {
    try {
      ...
      const allPurchaseProducts: IPurchaseProduct[] = [];
      const newPurchase = await db.transaction(async (tx: any) => {
        await tx
          .insert(purchases)
          .values({
            mr_id,
  ...
  retrieveByMrId: (async (req: Request, res: Response) => {
    try {
  ...
  updateByMrId: (async (req: Request, res: Response) => {
    try {
      const { mrId } = req.params;
      const {
        mr_id: mrIdBody,
      ...
      let updatedPurchase;
      const newlyPurchaseProducts: IPurchaseProduct[] = [];

      if (Array.isArray(products) && products.length > 0) {
        updatedPurchase = await db.transaction(async (tx: any) => {
          const [updatedPurchase] = await tx
            .update(purchases)
            .set(updatedData)
            .where(ilike(purchases.mr_id, mrId))
  ...
  deleteByMrId: (async (req: Request, res: Response) => {
    try {
      const { mrId } = req.params;
