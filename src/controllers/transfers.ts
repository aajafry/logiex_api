...
import { findInventoryByName, findTransferByTrfId } from "../services/index";
...
export const transfersController = {
  create: (async (req: Request, res: Response) => {
    try {
      ...
      let newTransfer;
      let allTransferProducts: object[] = [];
  ...
  retrieveByTrfId: (async (req: Request, res: Response) => {
    try {
  ...
  updateByTrfId: (async (req: Request, res: Response) => {
    try {
      const { trfId } = req.params;
      ...
      if (Array.isArray(products) && products.length > 0) {
        updatedTransfer = await db.transaction(async (tx: any) => {
          ...
          for (const product of products) {
            ...
            if (!sourceInventoryProductQuery) {
              throw new Error(
                `Product "${productName}" (${quantity} units) for MR "${mr_id}" not found in inventory "${existingTransfer.source_inventory}". Please check inventory products.`
              );
              tx.rollback();
            }
  ...
  deleteByTrfId: (async (req: Request, res: Response) => {
    try {
      const { trfId } = req.params;
