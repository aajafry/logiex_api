...
import { ISaleProduct } from "../interfaces/index";
import {
...
export const salesController = {
  create: (async (req: Request, res: Response) => {
    try {
      ...
      const allSaleProducts: ISaleProduct[] = [];
      const newSale = await db.transaction(async (tx: any) => {
        await tx
          .insert(sales)
          .values({
            bill_id,
  ...
  retrieveByBillId: (async (req: Request, res: Response) => {
    try {
  ...
  updateByBillId: (async (req: Request, res: Response) => {
    try {
      const { billId } = req.params;
      const {
        bill_id,
      ...
      let updatedSale;
      const newlySaleProducts: ISaleProduct[] = [];

      if (Array.isArray(products) && products.length > 0) {
        updatedSale = await db.transaction(async (tx: any) => {
          // Update sale details
          const [updatedSale] = await tx
            .update(sales)
            .set(updatedData)
  ...
  deleteByBillId: (async (req: Request, res: Response) => {
    try {
      const { billId } = req.params;
